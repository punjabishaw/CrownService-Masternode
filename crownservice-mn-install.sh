...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='cnsc.conf'
CONFIGFOLDER='/root/.cnsc'
COIN_DAEMON='cnscd'
COIN_CLI='cnsc-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='https://github.com/ServiceCrown/crown/releases/download/v1.0.0/linux-binaries.tgz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='cnsc'
COIN_EXPLORER='http://explorer.crownservice.io/'
COIN_PORT=17777
RPC_PORT=17778
...
function download_node() {
  ...
  wget -q $COIN_TGZ
  compile_error
  tar zxfp $COIN_ZIP >/dev/null 2>&1
  compile_error
  cd linux
  chmod +x $COIN_DAEMON
  chmod +x $COIN_CLI
